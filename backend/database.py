...
def get_db_connection():
    base_dir = os.path.join(os.getenv("APPDATA"), "ArquiKids")
    os.makedirs(base_dir, exist_ok=True)

    ruta_db = os.path.join(base_dir, "arquikids.db")
    print("RUTA DB:", ruta_db)

    nueva_db = not os.path.exists(ruta_db)
    ...
    if nueva_db:
        print("Creando base de datos con tablas y datos iniciales...")
        conn.executescript("""
        ...
        ('Procesador (CPU)', 'Es el cerebro de la computadora. Piensa y toma decisiones para hacer funcionar todo.', 'Es como tu cerebro, que piensa y decide qué hacer cuando juegas o haces tarea.', 'cpu.glb', 1),
        ('Memoria RAM', 'Guarda la información que estás usando en este momento, como los juegos abiertos.', 'Es como tu mesa de trabajo. Entre más grande, más cosas puedes tener encima al mismo tiempo.', 'ram.glb', 2),
        ('Tarjeta Gráfica (GPU)', 'Se encarga de dibujar todo lo que ves en la pantalla, como los personajes y colores.', 'Es como un artista que dibuja rapidísimo cada escena de tu videojuego favorito.', 'gpu.glb', 3),
        ('Disco Duro (SSD/HDD)', 'Guarda todos tus archivos, juegos y fotos de forma permanente, aunque apagues la PC.', 'Es como tu mochila o un gran archivador donde guardas todo para usarlo otro día.', 'disco_duro.glb', 4),
        ('Fuente de Poder (PSU)', 'Toma la electricidad del enchufe y se la da a cada componente con la fuerza justa.', 'Es como el corazón que bombea la energía (sangre) a todo el cuerpo para que se mueva.', 'psu.glb', 5),
        ('Refrigeración', 'Mantiene frío al procesador y otros componentes para que no se calienten demasiado.', 'Es como tomar agua fría o sudar cuando haces mucho ejercicio para no tener calor.', 'refrigeracion.glb', 6),
        ('Monitor', 'Es la pantalla donde ves todo lo que hace la computadora: juegos, videos y dibujos.', 'Es como la televisión de la computadora, te muestra todo lo que está pasando.', 'monitor.glb', 7),
        ('Mouse', 'Te ayuda a mover el cursor y a hacer clic en la pantalla para elegir cosas.', 'Es como tu dedo mágico que puede señalar y tocar cualquier cosa en la pantalla.', 'mouse.glb', 8);
        ...
        (1, '¿Para qué sirve la Tarjeta Gráfica (GPU)?', 'Enfriar los componentes,Procesar y mostrar imágenes en pantalla,Guardar archivos,Conectar el mouse y teclado', 'Procesar y mostrar imágenes en pantalla', 'Es el artista que dibuja cada personaje, escenario y explosión en tiempo real.'),
        ...
        (1, '¿Cuál es la diferencia entre la Memoria RAM y el Disco Duro?', 'La RAM es más grande que el Disco Duro,La RAM es temporal y el Disco Duro es permanente,El Disco Duro es el cerebro y la RAM la pantalla,No hay diferencia', 'La RAM es temporal y el Disco Duro es permanente', 'La RAM es como tu mesa de trabajo y el Disco Duro es como tu mochila.'),
        ...
        (1, 'El procesador (CPU) es muy importante porque...', 'Guarda todos tus archivos,Muestra las imágenes en la pantalla,Realiza todos los cálculos y decisiones,Conecta todos los componentes', 'Realiza todos los cálculos y decisiones', 'Es como el director de una orquesta: dice a cada músico qué hacer y cuándo.'),
        (1, '¿Qué componente es INDISPENSABLE para que la computadora reciba electricidad?', 'Tarjeta Gráfica,Disco Duro,Fuente de Poder (PSU),Refrigeración', 'Fuente de Poder (PSU)', 'Sin el corazón no llega la energía, igual que sin comida no tienes energía para jugar.'),
        (1, 'Si quisieras ARMAR una computadora, ¿cuál sería el PRIMER componente a instalar?', 'La Tarjeta Gráfica,El Disco Duro,La Tarjeta Madre (Placa Base),La Memoria RAM', 'La Tarjeta Madre (Placa Base)', 'Primero pones el esqueleto (tarjeta madre), luego le agregas los órganos.');
        """)
        conn.commit()
